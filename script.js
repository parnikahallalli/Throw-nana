...
window.addEventListener("keydown", (e) => {
  ...
  if (e.code === "Space") {
    ...
    if (!gameRunning && (lives <= 0 || remainingTime <= 0)) return resetGame();
...
function updateGameLogic() {
  ...
  if (keys.left) monkey.x -= monkey.speed;
  if (keys.right) monkey.x += monkey.speed;
  monkey.x = Math.max(40, Math.min(canvas.width - 40, monkey.x));

  if (keys.space) throwBanana();
  ...
  bananas = bananas.filter((b) => b.x > 0 && b.x < canvas.width && b.y < canvas.height);
